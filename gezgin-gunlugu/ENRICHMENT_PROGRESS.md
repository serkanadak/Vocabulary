# Yerel Arşiv Zenginleştirme Planı

Amaç: yerel arşivi zenginleştirmek — (a) mevcut mid-tier şehirlerin mekan
listelerini genişletmek, (b) turistik potansiyeli orta ama popüler güzergahların
ana ara/geçiş duraklarını eklemek (Sirmione, Graz gibi). Her batch: yeni yerler
+ mekanları, sonra doğrulama + commit + PR + merge + deploy. Batch bitince
sıradakine geç; hepsi bitince zamanlayıcıyı durdur.

İşaretleme: [x] tamamlandı, [ ] bekliyor. (Kaldığın yeri buradan ve veriden anla.)

## Ara/geçiş durakları (yeni yerler)

- [x] B1 — İtalya ara duraklar: Verona, Padova, Vicenza, Bologna, Bergamo, Como, Bolzano, Trento, Siena, San Gimignano, Lucca, Cinque Terre, Rimini
- [ ] B2 — Slovenya/Hırvatistan/Adriyatik: Maribor, Postojna, Piran, Rovinj, Pula, Plitvice, Split, Šibenik, Trogir, Opatija, Zagreb(var)
- [ ] B3 — Almanya ara duraklar: Regensburg, Nürnberg, Würzburg, Rothenburg, Füssen (Neuschwanstein), Heidelberg, Dresden, Leipzig, Baden-Baden, Freiburg
- [ ] B4 — İsviçre & Doğu Fransa: Bern, Cenevre, Lausanne, Montreux, Lugano, Strasbourg, Colmar, Dijon, Annecy, Avignon, Aix-en-Provence
- [ ] B5 — Orta Avrupa (Çekya/Slovakya/Macaristan/Avusturya): Brno, Český Krumlov, Salzkammergut(Wachau/Melk), Győr, Eger, Pécs, Szentendre, Balaton, Košice
- [ ] B6 — Yunanistan & Balkan ara: Meteora (Kalambaka), Delphi, Nafplio, Ioannina, Bitola, Trebinje, Ston, Kotor(var), Budva(var)

## Mevcut şehir mekanlarını genişletme (8+'a çıkar)

- [ ] E1 — İtalya: Napoli, Milano
- [ ] E2 — İspanya/Portekiz: Sevilla, Granada, Lizbon, Porto, Madrid
- [ ] E3 — Almanya/Orta Avrupa: Münih, Berlin, Selanik, Krakow, Varşova
- [ ] E4 — Asya üst sıra: Kyoto, Pekin, Bangkok, Singapur, Delhi, Dubai
- [ ] E5 — Amerika üst sıra: New York, Rio, Buenos Aires, Meksiko, Havana
- [ ] E6 — Afrika/Okyanusya: Marakeş, Cape Town, Sydney

Not: "var" = zaten arşivde. Tamamlanınca bu dosyayı da güncelle.
